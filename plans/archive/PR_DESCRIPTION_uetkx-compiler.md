...
- **Phase 7:** `UseTween`/`UseAnimate`/`UseTweenValue` real (host-clock, retarget-from-current, easing, self-re-arming frame chain) + `UseSfx` sink — **every hook now stub-free**; exit-animation Presence protocol designed (`plans/archive/EXIT_ANIMATION_DESIGN.md`). Asset brushes/focus/item-model → TD-022.
...
- **Finalization:** `ReactiveUI.Acceptance` (the coherence gate: drift/contract/registration/format-round-trip/HMR-swap/schema-parity in one suite) + `plans/archive/OWNER_ACCEPTANCE_CHECKLIST.md` (the interactive half).
