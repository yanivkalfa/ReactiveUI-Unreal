...
exports.classifyCursor = classifyCursor;
exports.insideNoncodeMarkup = insideNoncodeMarkup;
const codePoints_1 = require("./codePoints");
const cppScanner_1 = require("./cppScanner");
function classifyCursor(source, cpOffset) {
    const src = (0, codePoints_1.toCodePoints)(source);
    ...
    for (let i = at - 1; i >= 0; i--) {
        const c = src[i];
        if (c === 62 /*>*/)
            break; // a completed tag before the cursor
        if (c === 60 /*<*/) {
            openLt = i;
            break;
        }
        if (c === 10 && openLt === -1 && at - i > 400)
    ...
    for (let i = at - 1; i >= 0; i--) {
        ...
        if (c === 64 /*@*/) {
            ...
            if (onlyIdent)
                return { kind: "directive", partial: text(src, i + 1, at) };
            break;
        }
        if (!isIdent(c))
            break;
...
function text(src, start, end) {
    let out = "";
    for (let i = start; i < end; i++)
        out += String.fromCodePoint(src[i]);
    return out;
}
exports.DIRECTIVES = ["if", "elif", "else", "for", "while", "match", "case", "default"];
/** Suppress markup noise: is the offset inside a markup comment/string? */
function insideNoncodeMarkup(source, cpOffset) {
    const src = (0, codePoints_1.toCodePoints)(source);
    let i = 0;
    while (i < src.length && i < cpOffset) {
        const j = (0, cppScanner_1.skipNoncodeMarkup)(src, i);
        if (j !== i) {
            if (cpOffset > i && cpOffset < j)
                return true;
            i = j;
            continue;
        }
